    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-green-50">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 className="text-5xl md:text-7xl font-bold text-gray-800 leading-tight mb-6">
              Bienvenue dans<br />
              <span className="text-pink-600">le Jardin de Patricia</span>
            </h1>
            <p className="text-xl text-gray-600 mb-10">
              Un espace plein d'amour, de souvenirs et de petites fleurs pour une personne extraordinaire.
            </p>
            <div className="flex flex-wrap gap-4">
              ...
              <Link to="/souvenirs">
                <button className="px-8 py-4 rounded-2xl bg-gradient-to-r from-amber-400 to-pink-400 text-white font-semibold shadow-xl hover:scale-105 transition">
                  📚 Souvenirs
                </button>
              </Link>
            </div>
          ...
          <div>
            <img src={gardenHero} alt="Le Jardin de Patricia" className="w-full rounded-3xl shadow-2xl" />
          </div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-20">
        <div className="text-center mb-10">
          <h2 className="text-4xl font-bold text-pink-700">📸 Quelques souvenirs</h2>
          <p className="text-gray-600 mt-3">Quelques instants précieux du jardin</p>
        </div>
        <div className="grid md:grid-cols-3 gap-8">
          {galerie.map((photo, index) => (
            <div key={index} className="bg-white rounded-3xl shadow-xl overflow-hidden">
              <img src={photo} alt="Souvenir" className="w-full h-80 object-cover" />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-12">
        <div className="bg-white rounded-3xl shadow-xl p-10 text-center">
          <h2 className="text-3xl font-bold text-pink-700 mb-6">💌 Citation du jour</h2>
          <p className="text-2xl italic text-pink-600">{citation}</p>
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-12">
        <div className="bg-gradient-to-r from-pink-100 to-green-100 rounded-3xl shadow-xl p-10 text-center">
          {joursRestants > 0 ? (
            <>
              <h2 className="text-3xl font-bold text-pink-700 mb-4">🎂 Compte à rebours</h2>
              <p className="text-2xl">
                Patricia fête son anniversaire dans
                <span className="font-bold text-pink-600"> {joursRestants} jours</span>
              </p>
            </>
          ) : (
            <h2 className="text-4xl font-bold text-pink-700">🎉 Joyeux anniversaire Patricia !</h2>
          )}
        </div>
      </section>
